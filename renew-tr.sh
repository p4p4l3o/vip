...
users=$(grep -E "^### " "/etc/trojan-go/akun.conf" | cut -d ' ' -f 2 | sed -n "${CLIENT_NUMBER}"p)
expgo=$(grep -E "^### " "/etc/trojan-go/akun.conf" | cut -d ' ' -f 3 | sed -n "${CLIENT_NUMBER}"p)
now=$(date +%Y-%m-%d)
d1=$(date -d "$exp" +%s)
d2=$(date -d "$now" +%s)
exp2=$(( (d1 - d2) / 86400 ))
...
sed -i "s/### $users $expgo/### $users $exp4/g" /etc/trojan-go/akun.conf
clear
echo ""
echo " Trojan VPN Account Was Successfully Renewed"
echo " ==========================" | lolcat
